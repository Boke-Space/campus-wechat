        ...
        >
          ...
          <view class="listtitle">
            ...
            <view style="display:flex;margin-top: 8px;position:relative">             
              <van-image round lazy-load width="2rem" height="2rem" :src="avatarUrl" @click="gotoPerson(item.username)" />
              <view class="username">{{ item.username }}</view>
                <van-icon style="position:absolute; right: 10px; bottom: 4px;" name="delete-o" size="24px" @click="handleDelete(item.id)"/>
                <van-icon style="position:absolute; right: 40px; bottom: 4px;" name="notes-o" size="24px" @click="handleModify(item)" />
            </view>
          </view>
        </view>
      </view>
...
const getList = async (params = {}) => {
  const { data: { list, total } } = await getNotice({ username: username })
...
const gotoDetail = (item) => {
  uni.navigateTo({ url: `../Comment?item=${JSON.stringify(item)}`  })
...
const handleModify = (item: any) => uni.navigateTo({ url: '/components/Mine/NoteEdit?item=' + JSON.stringify(item) })

const handleDelete = async (id: number) => {
    await deleteNotice(id)
    const { data: { list } } = await getNotice({ username: username })
    noticeList = list
}
